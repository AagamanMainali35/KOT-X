...
class BillingSerializer(serializers.ModelSerializer):
    ...
    """
    Order_id=serializers.PrimaryKeyRelatedField(queryset=Order.objects.all(),source="Order_ins")
    ...
    def to_internal_value(self, data):
        ...
        if extra_feilds: 
            raise serializers.ValidationError({feild:"This field is not allowed." for feild in extra_feilds})
    ...
    def create(self,validated_data):
        ...
        if items:
            for item in items:
                print(f'{item.order_items.price}x{item.quantity}={item.order_items.price*item.quantity}')
